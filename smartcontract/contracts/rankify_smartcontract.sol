...
//     event ReviewSubmitted(uint256 id, string title, string description, uint256 rating, uint256 timestamp);
//     event QRCodeGenerated(uint256 qrCode, uint256 transactionId);
...
//     function submitReview(string memory _title, string memory _description, uint256 _rating, uint256 _qrCode) public {
//         require(!qrCodes[_qrCode].used, "QR code has already been used");
//         require(_qrCode < qrCodeCounter, "Invalid QR code");

//         reviews[reviewCounter] = Review(reviewCounter, _title, _description, _rating, block.timestamp);
//         qrCodes[_qrCode].used = true;

//         emit ReviewSubmitted(reviewCounter, _title, _description, _rating, block.timestamp);

//         reviewCounter++;
//     }

//     function getReviews(uint256 _id) public view returns (string memory, string memory, uint256, uint256) {
//         Review memory review = reviews[_id];
//         return (review.title, review.description, review.rating, review.timestamp);
//     }
// }
